...
def get_cylinder_cgo(start, end, radius, start_color, end_color):
    ...
    return [
        CGO.CYLINDER, *start, *end, radius, *start_color, *end_color
    ]
...
def get_cone_cgo(start, end, start_radius, end_radius,
    ...
    _check_color(end_color)
    return [
        CGO.CONE, *start, *end, start_radius, end_radius,
...
def get_sphere_cgo(pos, radius, color):
    ...
    _check_color(color)
    return [
        CGO.COLOR, *color, CGO.SPHERE, *pos, radius
    ]
...
def get_line_cgo(pos, color, width=1.0):
    ...
    return [CGO.LINEWIDTH, width, CGO.BEGIN, CGO.LINE_STRIP] \
           + vertices + [CGO.END]
...
def get_multiline_cgo(start, end, color, width=1.0):
    ...
    if len(start) != len(end):
        raise IndexError(
            f"{len(start)} start positions are given, "
            f"but {len(end)} end positions"
        )
    for p in start:
